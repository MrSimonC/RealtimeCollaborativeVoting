...
namespace GrantsCollaboration.MSGraph
{
    public class GraphServiceClientFactory
    {
        public static GraphServiceClient GetAuthenticatedGraphClient(Func<Task<string>> acquireAccessToken,
                                                                                 string baseUrl)
            => new GraphServiceClient(baseUrl, new CustomAuthenticationProvider(acquireAccessToken));
    }

    internal class CustomAuthenticationProvider : IAuthenticationProvider
    {
        public CustomAuthenticationProvider(Func<Task<string>> acquireTokenCallback) => acquireAccessToken = acquireTokenCallback;
